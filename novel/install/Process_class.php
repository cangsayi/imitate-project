<?php
	...
	class  Process {
		...
		private $info;
		//==========================================
		// 函数: __construct($configFile="../config.inc.php", $sqlFile="novel.sql")
		// 功能: 构造方法用于初使化对象的成员属性
		// 参数: configFile是指定需要修改的配置文件，sqlFile是需要安装数据表的SQL语句文件 
		// 返回: 无
		//==========================================	
		function __construct($configFile="../config.inc.php", $sqlFile="novel.sql"){
		...
		function createDb($user){
			...
			$insert="INSERT INTO ".TAB_PREFIX."admin(username,password, email) VALUES('".$user["ADMIN_USER"]."', '".sha1($user["ADMIN_PWD"])."','".$user["ADMIN_MAIL"]."')";
