<{include file="common/header.tpl" title="作品管理"}>
</head>
<{if !$smarty.session.aid}>
<div class="body-content">
    <form method="post" class="form-x" action="book.php"> 
    <h1 style=" margin: auto auto 50px 110px;">你还不是作者</h1>
    <input type="text" hidden name="userId" readonly value="<{$smarty.session.id}>" />   
    <div class="form-group">
        <div class="label">
          <label>用户昵称：</label>
        </div>
        <div class="field">
          <input type="text" class="input w50" name="uname" readonly value="<{$smarty.session.name}>" />         
          <div class="tips"></div>
        </div>
      </div>  
       <div class="form-group">
     ...
     <div class="form-group">
        ...
        <div class="field">
          <input name="submit" value="成为作者" class="button bg-main icon-check-square-o" type="submit" />
        </div>
      </div>
    </form>
  </div>
...
<table class="table table-hover text-center">
      <tr>
       ...
       <{section name=data loop=$rows }>   
        <tr>
          <td><{$rows[data].id}></td>
          <td><a href="chapter.php?bookId=<{$rows[data].id}>" title="进入章节详情"><{$rows[data].name}></a></td>
